...
const Modal = ({ modal }) => {
  ...
  const { logoUrl, title, subTitle, placeholder, button } = modal.content;

  const handleOnClick = () => {
    let copyModal = JSON.parse(JSON.stringify(modal));
    setTemplate(copyModal);
  };

  return (
    <Fragment>
      <div className="bg-indigo-300 h-auto w-auto absolute hover:opacity-100 ">
        SELECT
      </div>
      <div
        className="bg-gray-400 h-auto w-auto rounded-xl flex justify-center items-center hover:cursor-pointer"
        onClick={handleOnClick}
      >
        <div className="bg-white w-auto h-1/4 m-6  rounded-xl ">
            ...
            <div className="grid-cols-2 m-4 ">
              <button
                className={
                  ...
                  button.cancel.color +
                  "] text-" +
                  button.cancel.textColor +
                  " text-[" +
                  button.cancel.textSize +
              ...
              <button
                className={
                  "bg-[" +
                  button.continue.color +
                  "] text-" +
                  button.continue.textColor +
                  " text-[" +
                  button.continue.textSize +
                  "px]  border-2 rounded-lg m-4 p-2"
                }
              >
